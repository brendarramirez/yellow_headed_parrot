# Unlocking the Past to Protect the Future: Forensic DNA and Historical Specimens Reveal the Origins of Smuggled Yellow-headed Parrots

### Lead author Brenda Ramirez (brendarosaramirez@gmail.com)
### [DOI](https://doi.org/10.1111/eva.70282)

### Data Processing
- [SNP calling markdown](https://github.com/brendarramirez/yellow_headed_parrot/blob/main/amazona.oratrix.snp.calling.md): Calling SNPs from our Yellow-headed Parrot Ultra Conserved Element (UCE) sequences
- [SNP filtering markdown](https://brendarramirez.github.io/yellow_headed_parrot/amazona_oratrix_snp_filtering.html): Filtering called SNPs to remove outliers and low quality data. Includes code for both PCAs (all filtered samples & eastern Mexico subset) and the SplitsTree of all filtered samples

### Locator
- [Locator markdown](https://brendarramirez.github.io/yellow_headed_parrot/amazona_oratrix_locator.html): Predicting localities of Yellow-headed Parrots of unknown origin

### Quantifying pairwise relatedness
- [Calculating pairwise Fst between major lineages](https://brendarramirez.github.io/yellow_headed_parrot/YHPA.pairwise.fst.html)
- [Calculating pairwise kinship matrix among all samples](https://brendarramirez.github.io/yellow_headed_parrot/YHPA.relatedness.html)
- [Calculating pairwise Fst and fixed differences to identify fine-scale structure](https://brendarramirez.github.io/yellow_headed_parrot/yhpa.subset.fst.fixed.diffs.html)
- [Randomization procedure to calculate p-values associated with pairwise FST comparisons](https://brendarramirez.github.io/yellow_headed_parrot/FST.pvalue.randomization.html)
- [Comparing linked to unlinked SNPs for PCA making (Fig. S2)](https://brendarramirez.github.io/yellow_headed_parrot/YHPA.unlinked.PCA.html)
